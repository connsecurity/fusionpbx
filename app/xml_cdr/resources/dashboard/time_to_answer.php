...
//select the averages
    $sql = "
            ...
            (
                select
                    (answer_epoch - start_epoch) as tta, start_epoch
                from
                    v_xml_cdr
                where
	...
	<script>
		...
		const time_to_answer_chart_config = {
			...
			options: {
				...
				plugins: {
					chart_counter: {
						chart_text: '<?php echo number_format($result["today"], 1); ?>s'
					},
					legend: {
						display: false
					},
    ...
    foreach ($result as $period => $seconds) {
        ...
        echo "<td valign='top' class='".$row_style[$c]." hud_text' nowrap='nowrap'>".(($seconds != 0) ? number_format($seconds, 1) : "----")."s</td>\n";
        echo "</tr>\n";
    }    
