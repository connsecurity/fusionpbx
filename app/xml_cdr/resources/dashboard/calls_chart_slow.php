...
//get contact call group
	...
	$sql = "WITH recursive 
			...
			SELECT
				...
				count(case when (v.json->'variables'->>'bridge_epoch')::numeric = 0 then 1 end) as calls_missed,
				avg(case when (v.json->'variables'->>'bridge_epoch')::numeric > 0 then ((v.json->'variables'->>'bridge_epoch')::numeric - (v.json->'variables'->>'progress_epoch')::numeric) end) as time_to_answer
			FROM   
				Hours_CTE as h
			left join
				v_xml_cdr as v
