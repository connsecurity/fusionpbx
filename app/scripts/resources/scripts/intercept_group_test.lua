...
if ( session:ready() ) then
	...
	--get session variables
		...
		agent_uuid = session:getVariable("uuid");
end
...
if (uuid ~= nil) then
	if (session:getVariable("billmsec") == nil) then
		if (hostname == call_hostname) then
			...
			--log.noticef( "Intercepting %s", uuid);
			--log.noticef("Agent %s", agent_uuid);

			--session:execute("set", "call_uuid="..uuid);
			--log.noticef( "Setting call_uuid to %s", uuid);

			--session:execute("set", "direction=outbound");
			--log.noticef("Setting direction to outbound");



			
			command = "uuid_bridge " ..uuid.. " "..agent_uuid;
			log.noticef(command);
			did_it_go_well = api:executeString(command);
			log.noticef(did_it_go_well);
			api:executeString("uuid_setvar "..uuid.." answer_time "..os.time());
			session:execute("set", "hangup_after_bridge=true");

			--did_it_go_well = session:execute("transfer", uuid);
			--log.noticef(did_it_go_well);
