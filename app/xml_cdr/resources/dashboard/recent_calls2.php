...
//get the recent calls from call detail records
	$sql = "
		...
		and 
			...
			$sql .= "
			and start_epoch > ".strtotime('today')."
		order by
			start_epoch desc";
...
//add doughnut chart
	...
	unset($call_groups_extensions, $extensions_list, $sql, $parameters, $result, $num_rows, $index, $row);

	echo "</table>\n";
	echo "<span style='display: block; margin: 6px 0 7px 0;'><a href='".PROJECT_PATH."/app/xml_cdr/xml_cdr.php?direction=inbound&start_stamp_begin=".date('Y-m-d-H-i-s', strtotime('+0300'))."+00%3A00&start_stamp_end=".date('Y-m-d')."+23%3A59'>".$text['label-view_all']."</a></span>\n";
	echo "</div>";
	$n++;
